...
const constants_2 = require("../../ldap.ee/constants");
...
class Reset extends base_command_1.BaseCommand {
    async run() {
        ...
        const ldapIdentities = await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).find({
            where: { providerType: 'ldap' },
            select: ['userId'],
        });
        const personalProjectIds = await di_1.Container.get(project_relation_repository_1.ProjectRelationRepository).getPersonalProjectsForUsers(ldapIdentities.map((i) => i.userId));
        if (flags.projectId ?? flags.userId) {
            ...
            await di_1.Container.get(user_repository_1.UserRepository).manager.transaction(async (trx) => {
        ...
        const [ownedSharedWorkflows, ownedSharedCredentials] = await Promise.all([
            di_1.Container.get(shared_workflow_repository_1.SharedWorkflowRepository).find({
                select: { workflowId: true },
                where: { projectId: (0, typeorm_1.In)(personalProjectIds), role: 'workflow:owner' },
            }),
            di_1.Container.get(shared_credentials_repository_1.SharedCredentialsRepository).find({
        ...
        for (const { workflowId } of ownedSharedWorkflows) {
            await di_1.Container.get(workflow_service_1.WorkflowService).delete(owner, workflowId);
        }
        for (const credential of ownedCredentials) {
            await di_1.Container.get(credentials_service_1.CredentialsService).delete(credential);
        }
        await di_1.Container.get(auth_provider_sync_history_repository_1.AuthProviderSyncHistoryRepository).delete({ providerType: 'ldap' });
        await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).delete({ providerType: 'ldap' });
        await di_1.Container.get(user_repository_1.UserRepository).deleteMany(ldapIdentities.map((i) => i.userId));
        await di_1.Container.get(project_repository_1.ProjectRepository).delete({ id: (0, typeorm_1.In)(personalProjectIds) });
        await di_1.Container.get(settings_repository_1.SettingsRepository).delete({ key: constants_2.LDAP_FEATURE_NAME });
        await di_1.Container.get(settings_repository_1.SettingsRepository).insert({
    ...
    async getProject(userId, projectId) {
        if (projectId) {
            const project = await di_1.Container.get(project_repository_1.ProjectRepository).findOneBy({ id: projectId });
            if (project === null) {
        ...
        if (userId) {
            const project = await di_1.Container.get(project_repository_1.ProjectRepository).getPersonalProjectForUser(userId);
            if (project === null) {
