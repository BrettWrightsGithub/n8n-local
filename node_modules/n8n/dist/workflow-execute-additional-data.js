    ...
    if (fullRunData.data.resultData.error !== undefined) {
        ...
        if (errorWorkflow && !(mode === 'error' && workflowId && errorWorkflow === workflowId)) {
            ...
            di_1.Container.get(ownership_service_1.OwnershipService)
                ...
                .then((project) => {
                void di_1.Container.get(workflow_execution_service_1.WorkflowExecutionService).executeErrorWorkflow(errorWorkflow, workflowErrorData, project);
            })
                .catch((error) => {
            ...
            void di_1.Container.get(ownership_service_1.OwnershipService)
                .getWorkflowProjectCached(workflowId)
                .then((project) => {
                void di_1.Container.get(workflow_execution_service_1.WorkflowExecutionService).executeErrorWorkflow(workflowId, workflowErrorData, project);
            });
        }
    }
}
            ...
            async function (fullRunData) {
                ...
                if (externalHooks.exists('workflow.postExecute')) {
                    try {
                        ...
                        di_1.Container.get(n8n_core_1.ErrorReporter).error(error);
                        di_1.Container.get(n8n_core_1.Logger).error('There was a problem running hook "workflow.postExecute"', error);
                    }
                }
            },
        ],
...
async function getWorkflowData(workflowInfo, parentWorkflowId, parentWorkflowSettings) {
    ...
    if (workflowInfo.id !== undefined) {
        ...
        workflowData = await di_1.Container.get(workflow_repository_1.WorkflowRepository).get({ id: workflowInfo.id }, { relations });
...
async function startExecution(additionalData, options, executionId, runData, workflowData) {
    ...
    di_1.Container.get(event_service_1.EventService).emit('workflow-pre-execute', { executionId, data: runData });
    let data;
    try {
        await di_1.Container.get(permission_checker_1.PermissionChecker).check(workflowData.id, workflowData.nodes);
        await di_1.Container.get(subworkflow_policy_checker_service_1.SubworkflowPolicyChecker).check(workflow, options.parentWorkflowId, options.node, additionalData.userId);
