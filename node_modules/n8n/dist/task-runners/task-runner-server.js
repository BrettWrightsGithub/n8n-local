...
const task_runner_auth_controller_1 = require("../task-runners/auth/task-runner-auth.controller");
const task_runner_ws_server_1 = require("../task-runners/task-runner-ws-server");
let TaskRunnerServer = class TaskRunnerServer {
    ...
    configureRoutes() {
        const createRateLimiter = () => (0, express_rate_limit_1.rateLimit)({
            windowMs: 1000,
            limit: 5,
            message: { message: 'Too many requests' },
        });
        this.app.use(this.upgradeEndpoint, createRateLimiter(), this.taskRunnerAuthController.authMiddleware, (req, res) => this.taskRunnerWsServer.handleRequest(req, res));
        const authEndpoint = `${this.getEndpointBasePath()}/auth`;
        this.app.post(authEndpoint, createRateLimiter(), (0, response_helper_1.send)(async (req) => await this.taskRunnerAuthController.createGrantToken(req)));
...
exports.TaskRunnerServer = TaskRunnerServer = __decorate([
    ...
    __metadata("design:paramtypes", [n8n_core_1.Logger,
        ...
        task_runner_ws_server_1.TaskRunnerWsServer])
], TaskRunnerServer);
