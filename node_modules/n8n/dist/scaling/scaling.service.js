...
let ScalingService = class ScalingService {
    constructor(logger, errorReporter, activeExecutions, jobProcessor, globalConfig, executionRepository, instanceSettings, orchestrationService, eventService) {
        this.logger = logger;
        ...
        void this.queue.process(constants_2.JOB_TYPE_NAME, concurrency, async (job) => {
            try {
                if (!this.hasValidJobData(job)) {
                    throw new n8n_workflow_1.ApplicationError('Worker received invalid job', {
    ...
    hasValidJobData(job) {
        return (0, n8n_core_1.isObjectLiteral)(job.data) && 'executionId' in job.data && 'loadStaticData' in job.data;
    }
};
