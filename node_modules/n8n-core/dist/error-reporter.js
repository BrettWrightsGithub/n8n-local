...
let ErrorReporter = class ErrorReporter {
    ...
    async init({ beforeSendFilter, dsn, serverType, release, environment, serverName, }) {
    ...
    async beforeSend(event, hint) {
        ...
        if (this.isApplicationError(originalException)) {
            if (this.isIgnoredApplicationError(originalException))
                return null;
    ...
    isApplicationError(error) {
        return error instanceof n8n_workflow_1.ApplicationError;
    }
    isIgnoredApplicationError(error) {
        return error.level === 'warning';
    }
    extractEventDetailsFromApplicationError(event, originalException) {
